...
int main()
{
	...
	do
	{
		...
		//search through bubble sorted  array to see if account number is present
		BubbleSort(AccNums, SIZE);
...
void BubbleSort(int AccNums[], int size)
{
	bool swap;
	int temp;
	...
	do
	{
		...
		{
			if (AccNums[count] > AccNums[count + 1])
			{
				temp = AccNums[count];
				AccNums[count] = AccNums[count + 1];
				AccNums[count + 1] = temp;
				swap = true;
			}
		}
	} while (swap);
...
Enter an account number to search for
7895122
Account Number: 7895122 was found in element 14 of the array.
